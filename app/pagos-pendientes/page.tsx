...
export default function PagosPendientesPage() {
  ...
  const total = egresos.reduce((s, e) => s + (e.precioFactura || 0), 0);
      ...
      <div className="max-w-3xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className={`rounded-2xl p-5 flex items-center gap-4 ${total > 0 ? 'bg-amber-50 border border-amber-200' : 'bg-green-50 border border-green-200'}`}>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {egresos.length === 0
                ? 'Todo al día ✓'
                : `${egresos.length} ${egresos.length === 1 ? 'factura pendiente' : 'facturas pendientes'}`}
            </p>
          </div>
        </div>
        ...
        {egresos.length > 0 && (
          <p className="text-xs text-center text-gray-400">
            Tocá el círculo a la izquierda de cada factura para marcarla como pagada.
          </p>
        )}
      </div>
    </div>
